...
from suppervisedlearningproject.utils import BASE_DIR, DATA_DIR
...
• Bei stabilem Training: 3-5"""
    ...
    def on_tab_changed(self, index):
        ...
        if current_tab:
            ...
            # Wenn es der Ergebnisse-Tab ist, aktualisiere die Textfelder
            if index == 1 and hasattr(self, 'report_text'):
                ...
                if hasattr(self, 'model_select_combo') and self.model_select_combo.currentText():
                    ...
                    if model_key in self.training_results:
                        ...
                        self.update_confusion_matrix(self.training_results[model_key]['conf_matrix'])
                ...
                try:
                    with open(self.results_file, 'rb') as f:
                        ...
                        if not isinstance(loaded_results, dict):
                            print(f"Warnung: Geladene Trainingsergebnisse sind kein Dictionary, sondern {type(loaded_results)}")
                            self.training_results = {}
                        else:
                            ...
                            for key, model_data in loaded_results.items():
                                if not isinstance(model_data, dict):
                                    print(f"Warnung: Modell '{key}' hat ungültiges Format, wird übersprungen")
                                    continue
                                ...
                                if missing_keys:
                                    print(f"Warnung: Modell '{key}' fehlen Schlüssel: {missing_keys}, wird übersprungen")
                                    continue

                                # Prüfe, ob conf_matrix ein gültiges numpy-Array ist
                                if not isinstance(model_data['conf_matrix'], np.ndarray):
                                    print(f"Warnung: Konfusionsmatrix für Modell '{key}' ist kein numpy-Array, wird übersprungen")
                                    continue
                            ...
                            print(f"Trainingsergebnisse geladen: {len(self.training_results)} gültige Modelle von {len(loaded_results)} gefunden.")
                except (pickle.UnpicklingError, EOFError) as pe:
                    print(f"Fehler beim Entpacken der Trainingsergebnisse: {str(pe)}")
                    print("Die Datei mit den Trainingsergebnissen ist möglicherweise beschädigt.")
                    self.training_results = {}
            else:
                print("Keine gespeicherten Trainingsergebnisse gefunden.")
                self.training_results = {}
        except Exception as e:
            print(f"Fehler beim Laden der Trainingsergebnisse: {str(e)}")
    ...
    def save_training_results(self):
        ...
        try:
            ...
            with open(self.results_file, 'wb') as f:
                ...
                print(f"Trainingsergebnisse gespeichert: {len(self.training_results)} Modelle.")
        except Exception as e:
            print(f"Fehler beim Speichern der Trainingsergebnisse: {str(e)}")
